...
public class VerifyCodeServlet extends HttpServlet {
	public static final String SESSION_VERIFY_CODE = "web.VerifyCode";
	...
	public void service(HttpServletRequest req, HttpServletResponse resp) throws ServletException, java.io.IOException {
		...
		for (int i = 0; i < codeCount; i++) {
			...
			g.drawString(strRand, (i + 1) * x-7, codeY);
			// 将产生的四个随机数组合在一起。
			randomCode.append(strRand);
		}
		// 将四位数字的验证码保存到Session中。
		HttpSession session = req.getSession();
		session.setAttribute(SESSION_VERIFY_CODE, randomCode.toString());
		// 禁止图像缓存。
